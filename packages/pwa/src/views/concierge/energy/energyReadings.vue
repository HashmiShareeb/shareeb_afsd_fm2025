...
import { MY_ENERGY_READINGS } from '@/graphql/energy-reading.entity'
...
// --- Queries --- realtime fetching every 2 seconds
const { result: readingsResult, refetch: refetchReadings } = useQuery(
  MY_ENERGY_READINGS,
  () => ({ userId: userId.value }),
  { enabled: computed(() => !!userId.value), pollInterval: 2000 },
)
const energyReadings = computed(
  () => readingsResult.value?.myEnergyReadings ?? [],
)
